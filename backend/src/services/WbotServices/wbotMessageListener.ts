...
export const verifyMessage = async (
  ...
  // Se a conversa está em open, sem atendente humano, e estamos recebendo mensagem do cliente,
  // marcamos o ticket como controlado por IA/integração para não ser filtrado no frontend.
  if (!msg.key.fromMe && ticket.status === "open" && !ticket.userId) {
    const shouldMarkAsBot =
      ticket.isBot === true ||
      ticket.useIntegration === true ||
  ...
  if (
    ...
    if (!ticket.imported && ticket.status === "pending") {
      io.of(String(companyId))
        .emit(`company-${companyId}-ticket`, {
          action: "update",
          ticket,
...
const handleOpenAi = async (
  ...
  if (
    ...
    payload.top_p = (typeof integrationValue?.topP === "number") ? integrationValue.topP : 1;
    payload.presence_penalty = (typeof integrationValue?.presencePenalty === "number") ? integrationValue.presencePenalty : 0;
    payload.frequency_penalty = (typeof integrationValue?.frequencyPenalty === "number") ? integrationValue.frequencyPenalty : 0;
    if (Array.isArray(integrationValue?.stopSequences)) {
      payload.stop = integrationValue.stopSequences;
    } else if (typeof integrationValue?.stopSequences === "string" && integrationValue.stopSequences.trim()) {
    ...
    if (prompt.voice === "texto") {
      ...
      const chunksSource = paragraphs.length ? paragraphs : [response];
      for (const p of chunksSource) {
        if (p.length <= 600) {
          blocks.push(p);
        } else {
          let start = 0;
    ...
    payload2.top_p = (typeof integrationValue?.topP === "number") ? integrationValue.topP : 1;
    payload2.presence_penalty = (typeof integrationValue?.presencePenalty === "number") ? integrationValue.presencePenalty : 0;
    payload2.frequency_penalty = (typeof integrationValue?.frequencyPenalty === "number") ? integrationValue.frequencyPenalty : 0;
    if (Array.isArray(integrationValue?.stopSequences)) {
      payload2.stop = integrationValue.stopSequences;
    } else if (typeof integrationValue?.stopSequences === "string" && integrationValue.stopSequences.trim()) {
...
const handleMessage = async (
 ...
 if (existingMessage) {
  ...
  try {
    ...
    //openai na conexao
    logger.info(`[AI-CONNECTION] evaluating: ticket=${ticket.id} status=${ticket.status} imported=${!!ticket.imported} isGroup=${isGroup} fromMe=${msg.key.fromMe} promptId=${whatsapp.promptId} useAgentSettings=${(whatsapp as any)?.useAgentSettings === true} agentDisabled=${(whatsapp as any)?.agentDisabled === true}`);
    if (
      !ticket.imported &&
      !isGroup &&
      !msg.key.fromMe &&
      ...
      // Permitir IA tanto em pending quanto em open, desde que não haja atendente humano
      (!ticket.userId) &&
    ...
    if (
      ...
      // Encaminhar para fila quando IA na conexão NÃO está ativa:
      // - IA desabilitada explicitamente (agentDisabled === true) OU
      // - Sem Prompt e sem uso de configurações globais
      (
        ((whatsapp as any)?.agentDisabled === true) ||
        (isNil(whatsapp.promptId) && !((whatsapp as any)?.useAgentSettings === true))
      ) &&
