...
const CreatePromptService = async (promptData: PromptData): Promise<Prompt> => {
    const { name, apiKey, prompt, model, queueId, maxMessages, companyId } = promptData;
    ...
    try {
        await promptSchema.validate({ name, apiKey, prompt, model, queueId, maxMessages, companyId });
    } catch (err) {
