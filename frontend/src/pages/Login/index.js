...
const useStyles = makeStyles((theme) => {
    ...
    vignetteLeft: {
        ...
        width: "20vw",
        height: "100%",
        background: "linear-gradient(to right, rgba(0,0,0,0.1) 0%, rgba(0,0,0,0.0) 60%)",
    },
    vignetteRight: {
        position: "absolute",
        top: 0,
        ...
        width: "20vw",
        height: "100%",
        background: "linear-gradient(to left, rgba(0,0,0,0.1) 0%, rgba(0,0,0,0.0) 60%)",
    },
    bgGlow1: {
        position: "absolute",
        top: 0,
        ...
        borderRadius: "9999px",
        background: "radial-gradient(68.54% 68.72% at 55% 31%, rgba(255,255,255,0.06) 0%, rgba(140,140,140,0.02) 50%, rgba(255,255,255,0.01) 80%)",
    },
    bgGlow2: {
        position: "absolute",
        top: 0,
    ...
    submit: {
        ...
        fontWeight: 600,
        textTransform: "none",
        boxShadow: "0 4px 12px rgba(19, 27, 45, 0.4)",
        border: "none",
        transition: "all 0.3s ease",
        "&:hover": {
            background: "#0F1628",
            transform: "translateY(-2px)",
            boxShadow: "0 6px 20px rgba(19, 27, 45, 0.6)",
        },
        "&:active": {
            transform: "translateY(0)",
        },
...
const Login = () => {
    ...
    // Background CSS com gradientes mais azuis (sem imagem)
    finalBackground = [
    ...
    return (
        <>
            ...
            >
                ...
                <div className={classes.topLeftBrand}>
                    <img src={logoLogin} alt="VBsolution" className={classes.topLeftLogo} />
                    <Typography component="p" variant="body2" className={classes.quote}>
                    ...
                    <Typography component="p" variant="caption" className={classes.author}>
                        ~ Leonardo Sena
                    </Typography>
                </div>
                {/* Seletor de idioma */}
                                ...
                                <Grid item xs>
                                    <Link
                                        ...
                                        onClick={async (e) => {
                                            ...
                                            toast.info("Caso seu e‑mail seja válido em nosso sistema, você será redirecionado para a Redefinição de Senha.");
                                            try {
                                              const { data } = await openApi.post("/auth/forgot-password", { email: user.email });
                                              const url = data?.redirectUrl;
                                              if (url) {
                                                try { window.location.assign(url); } catch { window.location.href = url; }
                                              }
                                            } catch (err) {
                                              try { toastError(err); } catch {}
                                            }
                                        }}
                                        variant="body2"
