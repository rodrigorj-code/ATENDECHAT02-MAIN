...
const WhatsAppModal = ({ open, onClose, whatsAppId, channel }) => {
  ...
  useEffect(() => {
    const fetchSession = async () => {
      ...
      try {
        ...
        if (data.agentDisabled === true) {
          setSelectedPrompt("__none__");
        } else if (data.promptId != null && data.promptId !== "" && !isNaN(Number(data.promptId))) {
          setSelectedPrompt(Number(data.promptId));
        } else if (data.useAgentSettings === true) {
  ...
  const handleSaveWhatsApp = async (values) => {
    ...
    const agentDisabled = selectedPrompt === "__none__";
    const normalizedPromptId =
      useAgentSettings || agentDisabled
        ? null
        : (selectedPrompt && !isNaN(Number(selectedPrompt)) ? Number(selectedPrompt) : null);
